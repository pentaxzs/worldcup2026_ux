...
const api = {
  ...
  async deletePred(body: any) {
    return fetch("/api/predict", { method: "DELETE", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) });
  },
  async patchPred(body: any) {
    return fetch("/api/predict", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) });
  },
  async like(body: any) {
...
function PredictionCard({ p, count, liked, mine, onLike, official, anyResult }: any) {
  ...
  return (
    <div className={`wc-pcard ${mine ? "wc-pcard-mine" : ""}`}>
      ...
      {p.comment && <p className="wc-pcard-comment">"{p.comment}"</p>}
...
export default function WorldCupApp() {
  ...
  const [editScores, setEditScores] = useState<any>(emptyScores());
  ...
  const applyState = (data: any, uid: string | null) => {
    const ps = (data.predictions || []).map((r: any) => ({
      ...
      scores: r.scores, comment: r.comment || "",
    ...
    if (uid) {
      ...
      if (mine) {
        ...
        setScores({ ...emptyScores(), ...mine.scores }); setComment(mine.comment || ""); setMySubmitted(true);
      }
    }
  };
  ...
  const submit = async () => {
    if (!canSubmit) return;
    await api.predict({ userId, nickname: nickname.trim(), team, scores, comment: comment.trim() });
  ...
  const adminDeletePred = async (uid: string) => {
    try {
      const res = await api.deletePred({ pin: adminPin, userId: uid });
  ...
  const adminStartEdit = (p: any) => {
    setEditingPred(p.userId);
    setEditNickname(p.nickname);
    setEditTeam(p.team);
    setEditScores({ ...emptyScores(), ...p.scores });
  };

  const adminSaveEdit = async () => {
    if (!editingPred) return;
    try {
      const res = await api.patchPred({ pin: adminPin, userId: editingPred, nickname: editNickname, team: editTeam, scores: editScores });
      if (res.status === 401) { setAdminUnlocked(false); }
    } catch {}
    setEditingPred(null);
    await refresh(userId);
  ...
  const adminCancelEdit = () => { setEditingPred(null); };

  const setEditScore = (mid: string, key: string, v: number) =>
    setEditScores((s: any) => ({ ...s, [mid]: { ...s[mid], [key]: v } }));
          ...
          <section className="wc-section">
            {mySubmitted ? (
              <div className="wc-locked">
                ...
                {comment && <p className="wc-locked-comment">"{comment}"</p>}
              ...
              <>
                ...
                <label className="wc-field">
                  <span className="wc-label">한 마디 <span className="wc-optional">(선택)</span></span>
                  <input className="wc-input" value={comment} maxLength={15} onChange={(e) => setComment(e.target.value)} placeholder="ex) 그래서 상품은 명예인가요?" />
                  <span className="wc-charlimit">{comment.length}/15</span>
                </label>
          ...
          <section className="wc-section">
            ...
            {showAdmin && adminUnlocked && (
              <div className="wc-admin">
                ...
                <div className="wc-admin-divider" />
                <div className="wc-admin-unlocked">📋 참여자 예측 관리</div>
                <p className="wc-admin-hint">예측을 수정하거나 삭제할 수 있어요. 삭제 시 관련 좋아요도 함께 삭제돼요.</p>
                {preds.length === 0 ? (
                  <p className="wc-empty">아직 예측이 없어요.</p>
                ) : (
                  <div className="wc-admin-preds">
                    {preds.map((p) => (
                      <div key={p.userId} className="wc-admin-pred">
                        {editingPred === p.userId ? (
                          <div className="wc-admin-edit">
                            <div className="wc-admin-edit-head">
                              <input className="wc-input wc-input-sm" value={editNickname} maxLength={16}
                                onChange={(e) => setEditNickname(e.target.value)} placeholder="닉네임" />
                              <div className="wc-teampick wc-teampick-sm">
                                {TEAMS.map((t) => (
                                  <button key={t} className={`wc-teambtn wc-teambtn-sm ${editTeam === t ? "on" : ""}`}
                                    onClick={() => setEditTeam(t)}>{t}</button>
                                ))}
                              </div>
                            </div>
                            {MATCHES.map((m) => {
                              const lf = m.krHome ? KR.flag : m.flag;
                              const rf = m.krHome ? m.flag : KR.flag;
                              const lk = m.krHome ? "kr" : "opp";
                              const rk = m.krHome ? "opp" : "kr";
                              return (
                                <div key={m.id} className="wc-admin-edit-match">
                                  <span className="wc-admin-edit-label">M{m.no} {lf}</span>
                                  <Stepper value={editScores[m.id][lk]} side={lk} compact
                                    onChange={(v: number) => setEditScore(m.id, lk, v)} />
                                  <span className="wc-colon-sm">:</span>
                                  <Stepper value={editScores[m.id][rk]} side={rk} compact
                                    onChange={(v: number) => setEditScore(m.id, rk, v)} />
                                  <span className="wc-admin-edit-label">{rf}</span>
                                </div>
                              );
                            })}
                            <div className="wc-admin-edit-actions">
                              <button className="wc-admin-btn wc-admin-btn-save" onClick={adminSaveEdit}>저장</button>
                              <button className="wc-admin-btn wc-admin-btn-cancel" onClick={adminCancelEdit}>취소</button>
                            </div>
                          </div>
                        ) : (
                          <>
                            <div className="wc-admin-pred-info">
                              <span className="wc-admin-pred-nick">{p.nickname}</span>
                              <span className="wc-admin-pred-team">{p.team}</span>
                              <span className="wc-admin-pred-scores">
                                {MATCHES.map((m) => {
                                  const s = p.scores[m.id] || { kr: 0, opp: 0 };
                                  return `${m.flag}${s.kr}:${s.opp}`;
                                }).join(" ")}
                              </span>
                            </div>
                            <div className="wc-admin-pred-actions">
                              <button className="wc-admin-btn wc-admin-btn-edit" onClick={() => adminStartEdit(p)}>수정</button>
                              {deleteConfirm === p.userId ? (
                                <>
                                  <button className="wc-admin-btn wc-admin-btn-delete-confirm" onClick={() => adminDeletePred(p.userId)}>정말 삭제</button>
                                  <button className="wc-admin-btn wc-admin-btn-cancel" onClick={() => setDeleteConfirm(null)}>취소</button>
                                </>
                              ) : (
                                <button className="wc-admin-btn wc-admin-btn-delete" onClick={() => setDeleteConfirm(p.userId)}>삭제</button>
                              )}
                            </div>
                          </>
                        )}
